...
void Frame::meshFromPoints(bool draw_normals) {
	...
	for (int y = host.step;
		...
		for (int x = host.step;
			...
			ofColor color;
			if (draw_normals) {
				ofColor color(abs(normal.x) * 255.0,
						abs(normal.y) * 255.0,
						abs(normal.z) * 255.0);
			} else {
...
void Frame::drawNormals() {

	// draw normals with lines
	ofPoint p, normal;
	ofSetColor(ofColor::white);
	ofSetLineWidth(1.0);
	for (int y = 0; y < host.height; y += host.step) {
		for (int x = 1; x < host.width; x += host.step) {
			host.getPoint(&p, x, y);
			host.getNormal(&normal, x, y);
			if (normal.z != 0.0) {
				ofLine(p, (p + normal));
